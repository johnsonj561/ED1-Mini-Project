...
const int SWING_ARM_DELAY = 2300;     //total time delay to move object from color detection zone to loading zone
...
//variables to detect color retesting. Retesting must be done when there is no sig difference between colors
//we see this with green and blue sometimes. Since re-testing moves the block forward slightly, we need
//to adjust the delay time required to move block from color zone to loading zone
bool reTest = false;
int reTestDelay = 50;
int reTestCounter = 0;
...
void loop() {
  ...
  switch (colorDetected) {
    case ColorDetector::RED_MATCH:          //RED OBJECT DETECTED
      ...
      mBeltMotorDriver.driveForward();      
      delay(SWING_ARM_DELAY - reTestDelay*reTestCounter);
    ...
    default:
      ...
      reTest = true; 
      reTestCounter++;
      Serial.println("move forward for reTest");
      mBeltMotorDriver.driveForward();
      delay(100);
      Serial.println("stopping motor for reTest");
      mBeltMotorDriver.stopMotor();
  }
